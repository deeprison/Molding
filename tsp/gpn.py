...
class Attention(nn.Module):
    def __init__(self, n_hidden, device):
        ...
        v  = torch.FloatTensor(n_hidden).to(self.device)
...
class GPN(torch.nn.Module):
    ...
    def __init__(self, n_feature, n_hidden, device):
        ...
        # pointer layer
        self.pointer = Attention(n_hidden, device)
        
        # lstm encoder
        self.encoder = LSTM(n_hidden)
        
        # trainable first hidden input
        h0 = torch.FloatTensor(n_hidden).to(self.device)
        c0 = torch.FloatTensor(n_hidden).to(self.device)
        ...
        r1 = torch.ones(1).to(self.device)
        r2 = torch.ones(1).to(self.device)
        r3 = torch.ones(1).to(self.device)
